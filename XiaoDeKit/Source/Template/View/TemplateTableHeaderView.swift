//
//  TemplateTableHeaderView.swift
//  XiaoDeKit
//
//  Created by 小唐 on 09/05/2018.
//  Copyright © 2018 XiaoDeStudio. All rights reserved.
//

import UIKit

protocol TemplateTableHeaderViewProtocol: class {
    
    ///
    func templateHeaderView(_ headerView: TemplateTableHeaderView, didClickedDone doneView: UIButton) -> Void
    
}

///  
class TemplateTableHeaderView: UIView {
    ...
    /// 回调处理
    weak var delegate: TemplateTableHeaderViewProtocol?
    var doneClickAction: ((_ templateView: TemplateTableHeaderView, _ doneView: UIButton) -> Void)?
...
extension TemplateTableHeaderView {

    class func loadXib() -> TemplateTableHeaderView? {
        return Bundle.main.loadNibNamed("TemplateTableHeaderView", owner: nil, options: nil)?.first as? TemplateTableHeaderView
    }
...
extension TemplateTableHeaderView {
    ...
    @objc fileprivate func doneBtnClick(_ doneBtn: UIButton) -> Void {
        print("TemplateTableHeaderView doneBtnClick")
        guard let model = self.model else {
            return
        }
        self.delegate?.templateHeaderView(self, didClickedDone: doneBtn)
